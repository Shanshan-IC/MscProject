...
BEGIN_MESSAGE_MAP(CPUimageView, CView)
	...
	ON_COMMAND(ID_STATISTICS_HISTOGRAMSTATISTICS, &CPUimageView::OnStatisticsHistogramstatistics)
	ON_COMMAND(ID_STATISTICS_IMAGESTATISTICS, &CPUimageView::OnStatisticsImagestatistics)
	ON_COMMAND(ID_HISTOGRAM_EQUALIZE, &CPUimageView::OnHistogramEqualize)
	ON_COMMAND(ID_HISTOGRAM_EQUALIZATION, &CPUimageView::OnHistogramEqualization)
	ON_COMMAND(ID_PROCESS_AUTOADJUSTMENT, &CPUimageView::OnProcessAutoadjustment)
...
CPUimageDoc* CPUimageView::GetDocument() const // ·Çµ÷ÊÔ°æ±¾ÊÇÄÚÁªµÄ
{
...
void CPUimageView::OnStatisticsHistogramstatistics()
{
...
void CPUimageView::OnStatisticsImagestatistics()
{
	//char* texts[] = "real";

	MessageBox(_T("hahha"), _T("Image Info"),
		MB_OK);
}

void CPUimageView::OnHistogramEqualize()
{
	// TODO: Add your command handler code here
}


void CPUimageView::OnHistogramEqualization()
{
	if (!m_bIsEditMode || m_nPos == -1)
	{
		MessageBox(L"Please open the image file, and choose the edit mode");
	...
	ResetImage();
	HistogramEqualization(m_pImageBuffer, m_pImageTempBuffer, m_nPicWidth, m_nPicHeight);
}


void CPUimageView::OnProcessAutoadjustment()
{
	if (!m_bIsEditMode || m_nPos == -1)
	{
		MessageBox(L"Please open the image file, and choose the edit mode");
...
void CPUimageView::OnHistogramContrastlimitedadaptiveequalizaiton()
{
	...
	ResetImage();
	HistogramEqualizationContrast(m_pImageBuffer, m_pImageTempBuffer, m_nPicWidth, m_nPicHeight);
}
